...
static bool
char_zconsole_open(Chardev *chr, ChardevBackend *backend, Error **errp)
{
	struct termios tty;
	int fd;
	...
	qemu_chr_open_fd(chr, fd, fd, errp);

	return true;
}

static void
char_zconsole_init(ObjectClass *oc, const void *data)
{
	ChardevClass *cc = CHARDEV_CLASS(oc);

	cc->chr_open = char_zconsole_open;
}
